...
for i in $(seq 1 31); do
    ...
    mysql -u $user -p matomo --password=$pass -e "SELECT matomo_log_visit.idvisit as visitId, server_time as serverTime, config_browser_name as browserName, config_browser_version as browserVersion, inet_ntoa(conv(hex(location_ip), 16, 10)) as ip, conv(hex(matomo_log_visit.idvisitor), 16, 16) as visitorId, matomo_log_action.idaction as actionId, name as actionName from matomo_log_link_visit_action LEFT JOIN matomo_log_visit on matomo_log_visit.idvisit = matomo_log_link_visit_action.idvisit LEFT JOIN matomo_log_action on matomo_log_action.idaction = matomo_log_link_visit_action.idaction_url WHERE matomo_log_visit.idsite = $idsite AND server_time >= '$start' AND server_time < '$end' order by visitorId;" > "results_$start.tsv"
done
